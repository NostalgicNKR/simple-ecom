<?php $titleOfPage = isset($_GET['category']) ? $_GET['category'] : 'All Products'; ?>

<div class="container-fluid scrollarea mb-10 pb-5">
 <p class="h4 mx-3 mt-3"><?= isset($_GET['category']) ? $_GET['category'] : 'All Products' ?></p>
    <div class="row">
	<?php foreach ($products as $product) { ?>
        <div class="col-sm-3 mt-3">
            <div class="card card-effect" style="width: 18rem;">
			  <a href="<?= $base_url ?>customer?page=product-details.php&product=<?= $product["id"] ?>" class="text-decoration-none text-black"><img class="card-img-top" src="../uploads/images/<?= $product["image_url"] ?>" width="300px" height="250px" alt="Card image cap"></a>
			  <div class="card-body">
				...
				?>
					...
					<?php } ?>
						<a href="<?= strtok($_SERVER["REQUEST_URI"], '?') ?>?add-to-wishlist=<?= $product["id"] ?>" class="btn btn-outline-danger"><i class="fa-regular fa-heart"></i></a>
					</div>
				</div>
